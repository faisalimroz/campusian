...
const Allscholarship = () => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:5000/scholar')
    ...
    const handleDelete = async (scholarId) => {
        console.log('delete',scholarId)
        try {
            // Make a DELETE request to your backend to delete the blog
            const response = await fetch(`http://localhost:5000/scholar/${scholarId}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                // If the deletion was successful, update the state
                setUserBlogs(prevBlogs => prevBlogs.filter(blog => blog._id !== scholarId));
            } else {
    ...
    return (
        <div>
            <h1 className='text-center text-4xl'>All Scholarship</h1>
            <ul>
