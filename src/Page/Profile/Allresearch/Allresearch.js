...
const Allresearch = () => {
    ...
    const handleDelete = async (researchId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/researches/${researchId}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                // If the deletion was successful, update the state
                setUserBlogs(prevBlogs => prevBlogs.filter(blog => blog._id !== researchId));
            } else {
    ...
    return (
        <div>
            <h1 className='text-center text-4xl'>All Research Paper</h1>
            <ul>  
